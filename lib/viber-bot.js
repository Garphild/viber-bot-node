...
ViberBot.prototype._sendMessages = function (userProfile, messages, requestType, optionalTrackingData, optionalChatId) {
	...
	_.each(messages, message => {
		...
		promise = promise.then(() => new Promise((resolve, reject) => {
			if (requestType == REQUEST_TYPE.SEND_MESSAGE) return self._sendMessageFromClient(userProfile, message, optionalTrackingData || null, null, optionalChatId, message.minApiVersion).then(response => resolveCallback(response, message, resolve, reject), error => reject(error));
