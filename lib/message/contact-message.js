...
const REQUIRED_ARGUMENTS = ["contactName", "contactPhoneNumber", "text"];

function ContactMessage(contactName, contactPhoneNumber, optionalAvatar, optionalKeyboard, optionalTrackingData, timestamp, token, minApiVersion, text) {
...
ContactMessage.fromJson = function(jsonMessage, timestamp, token) {
	return new ContactMessage(jsonMessage.contact.name, jsonMessage.contact.phone_number,
		null, jsonMessage.tracking_data, timestamp, token, null, null, jsonMessage.text || null);
};
